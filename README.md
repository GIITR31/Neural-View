# Neural-View  
**BYOP 2025 – DSG IIT Roorkee**

Neural-View is a from-scratch implementation of **Neural Radiance Fields (NeRF)** built as part of my BYOP project.  
The aim of this project is to understand NeRF end-to-end by implementing **every stage manually** — from preprocessing and camera pose estimation to training, novel view synthesis, orbit video generation, and mesh extraction.
...
Neural-View/
├── FULL_NERF/
│   └── Full NeRF implementation (coarse + fine networks)
│
├── TINY_NERF/
│   └── Tiny-NeRF implementation (single MLP, faster training)
│
├── preprocessing/
│   └── COLMAP pipeline and scripts to generate transforms.json
│
├── Statue Test Data/
│   └── Real-world image dataset (images only, no transforms.json)
│
├── Test Videos/
│   └── Orbit videos generated by trained NeRF models
│
├── lego_testdata.zip
│   └── Synthetic NeRF-style dataset (images only)
│
├── EndTermReport.pdf
...
This section explains how to use **Neural-View** end-to-end — from raw images to final 3D outputs.
...
This step bridges **raw images → NeRF-ready data**.
...
3. Go to **Notebook Settings → Accelerator**  
4. Select **GPU** and choose **2× T4 GPUs**  
 <img width="476" height="128" alt="Screenshot 2026-01-05 at 10 45 28 PM" src="https://github.com/user-attachments/assets/6f42aa10-1c1e-4d0c-951f-edd0c47d6c09" />
...
## Step 4: 360° Orbit Video Generation
...
- The NeRF’s **density (sigma)** is queried
- A Gaussian filter smooths the density field
- **Marching Cubes** extracts a surface mesh

### Output:
<img width="400" height="400" alt="Screenshot 2026-01-05 at 10 55 36 PM" src="https://github.com/user-attachments/assets/b8e6aa3c-51d5-4081-8d5f-11ba1a72b58a" />

- `.ply` mesh file
- Can be opened in **Blender**, **MeshLab**, etc.

This step converts NeRF’s **implicit representation** into an **explicit 3D mesh**.
